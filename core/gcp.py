...
class Bigquery(object):
    ...
    def bqtable2gs(
        ...
        if destination_format == NEWLINE_DELIMITED_JSON:
            ...
            raise ValueError(
                f"Destination format {destination_format} not supported. Use NEWLINE_DELIMITED_JSON or CSV"
            )
    ...
    def load(
        ...
        date_partition_expiration_days=None,
    ):
        """
        ...
        if date_partition_column:
            # set partitioning
            if date_partition_expiration_days:
                expiration_ms = date_partition_expiration_days * 86400000
            else:
                expiration_ms = None
    ...
    def export_query_to_csv(
        ...
        if split_files:
            ...
            gs_file_name = file_name + "_*"
        else:
            path = temp_path
            gs_file_name = file_name
        ...
        try:
            self.delete_blobs(bucket_name=temp_bucket, path=gs_path)
        except Exception:
            pass
        self.bqtable2gs(
            dataset_name=temp_dataset,
        ...
        blobs = self.list_blobs(bucket_name=temp_bucket, path=gs_path)
