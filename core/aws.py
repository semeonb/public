...
class SQS(object):
    ...
    def send_list(self, data, queue_url, batch=False):
        if batch:
            return self.send_message_list(data=data, queue_url=queue_url)
        else:
            return self.send_batch_list(data=data, queue_url=queue_url)
